...
public class HostSampleService extends HostApduService {
    ...
    final static int APDU_UPDATE_LC = 4;
    ...
    private final static String ALL = "AllHostSampleService";
...
//    };
    ...
    public HostSampleService() {
        ...
        NdefMessage ndef = createUriMessage(3, "etf.unsa.ba/");
    ...
    public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
        ...
        for (int i = 0; i < commandApdu.length; i++) {
            Log.d(ALL, Integer.toHexString(commandApdu[i] & 0xff) + ' ' + (char)commandApdu[i]);
        }
        ...
        switch (commandApdu[APDU_INS]) {
            ...
            case INS_UPDATE:
                Log.d(TAG, "UPDATE");
                int offset = ((commandApdu[APDU_P1]) << 8) | commandApdu[APDU_P2];
                byte [] readData = new byte[commandApdu[APDU_UPDATE_LC] - offset];
                System.arraycopy(commandApdu, offset + 5, readData, 0, commandApdu[APDU_UPDATE_LC] - offset);
                Log.d(TAG, new String(readData));
